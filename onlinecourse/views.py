...
def submit(request, course_id):
    ...
    print('Form submission data: ', request.POST)
    choice_ids = []
    for key in request.POST.keys():
        if key.startswith('choice_'):
            choice_ids.append(request.POST[key])
    print('Selected Choice Is: ', choice_ids)
    for choice_id in choice_ids:
        submission.choices.add(Choice.objects.get(pk=choice_id))
    
    #submission.choices.set(request.POST.values())
    #submission.save()
...
def show_exam_result(request, course_id, submission_id):
    ...
    context['grade'] = round((grade / total_score)*100)
